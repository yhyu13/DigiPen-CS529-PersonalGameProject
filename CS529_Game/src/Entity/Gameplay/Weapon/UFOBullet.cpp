...
int My::Entity::UFOBullet::Tick(double deltaTime)
{
	if (deltaTime != 0.0)
	{
...
void My::Entity::UFOBullet::OnComponentHit(std::weak_ptr<Event> e)
{
	static int count = 0;
	...
	if (other->GetType() == MYGAMEOBJECT_TYPE::PLAYER)
	{
		...
		{
			// 1. Player flashing
			static_cast<Component::Sprite*>(other->GetComponent(COMPONENT_TYPE::SPRITE))->StartHitFlash();
			// 1. Player HP Reducing
			{
				...
				{
				case 1:
					damage = UFO_BULLET_BASE_DAMAGE;
					break;
				case 2:
					damage = UFO_BULLET_BASE_DAMAGE * 1.25f;
					break;
				case 3:
					damage = UFO_BULLET_BASE_DAMAGE * 1.75f;
					break;
				default:
					break;
				}
				hp_com->Reduce(damage);
			}
		}
