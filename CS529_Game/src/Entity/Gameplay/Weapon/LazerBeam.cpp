...
int My::Entity::LazerBeam::Tick(double deltaTime)
{
	if (deltaTime != 0.0)
	{
...
void My::Entity::LazerBeam::OnComponentHit(std::weak_ptr<Event> e)
{
	static int count = 0;
	...
	if (other->GetType() == MYGAMEOBJECT_TYPE::UFO)
	{
		...
		DEBUG_wPRINT(str2wstr(other->GetName()) << "\n");
		{
			...
			{
			case 1:
				damage = WEAPON_LAZER_BULLE_DAM_BASE;
				break;
			case 2:
				damage = WEAPON_LAZER_BULLE_DAM_BASE * 1.25f;
				break;
			case 3:
				damage = WEAPON_LAZER_BULLE_DAM_BASE * 1.75f;
				break;
			default:
				break;
			}
