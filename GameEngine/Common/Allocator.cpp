...
void My::Allocator::Reset(size_t data_size, size_t page_size, size_t alignment) noexcept
{
	...
	assert(m_szBlockSize == (size_t)uint16_t(m_szBlockSize));
	...
	// Storing m_szDataSize as uint16_t right before each block
	m_szPageSize += sizeof(uint16_t) * m_nBlocksPerPage;
}
...
void* My::Allocator::Allocate() noexcept
{
    ...
    if (!m_pFreeList) {
        ...
        for (uint32_t i = 1; i < m_nBlocksPerPage; i++) {
			...
			// Storing m_szDataSize as uint16_t right before each block
			*(reinterpret_cast<uint16_t*>(pBlock) - 1) = (uint16_t)m_szDataSize;
...
My::BlockHeader* My::Allocator::NextBlock(BlockHeader *pBlock) noexcept
{
	...
	// Storing m_szBlockSize as uint16_t right before each block 
	return reinterpret_cast<BlockHeader *>(reinterpret_cast<uint8_t*>(pBlock) + m_szBlockSize + sizeof(uint16_t));
}
